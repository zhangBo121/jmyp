            ...
            if (id == item.id) {
                ...
                div.innerHTML = `
                        ...
                        <div class="main_right">
                            ...
                            <p><span id='${item.id}' class='add'>加入购物车</span></p>
                        </div>
                    </div>
                    <div class="g_detail">
                        <h2>商品详情</h2>
...
// 放到函数里去执行，原因：这些程序只能在if判断id 成功后执行
function glass() {
    // 放大镜
    ...
    // 给加入购物车按钮添加点击事件
    var add = document.querySelector('.add') //加入购物车按钮
    console.log(add);
    add.onclick = function() {
        ...
        if (localStorage.getItem('goods')) {
            ...
            // 本地没有数据时，就是一个空，相当于一个空数组
            var goodsArr = []
        }

        console.log(localStorage.getItem('goods'));

        // 设置一个监听器，判断本地中是否存有当前这一条数据，没有就是false
        var hasGoods = false

        // 判断当前选中的商品是否在购物车中(本地是否有这条数据)
        if (goodsArr.length > 0) {
            // 大于 0 ，说明本地有数据，购物车可以拿到
            goodsArr.forEach((item) => {
                console.log(item.id);
                // 用户加入购物车两次时，不是加入两条数据，而是在原有数据的基础上加 1
                if (item.id == id) { //判断购物车中是否存在
                    // 存在，数值加 1 ，不存在就加一条数据
                    item.num++ //每点击一次，数值 +1
                        hasGoods = true //开关为true 说明购物车有数据
                    return false //数值增加完，结束当前遍历，节省性能
                }
            })
        }

        // 购物车中没有当前选中的商品，则在本地添加一条数据
        if (!hasGoods) { //取反原因，没有就是false，执行else，取反会执行true
            goodsArr.push({ id: id, num: 1 })
        }

        // 更新本地存储的数据，本地的数据就是字符串

...
function dataDemand() {
    ...
    wrap.onclick = function(e) { //事件委托
        ...
        // -----------------------------------------------------------------------------------------------------------------------------
        //1.  主要是找出当前选中商品的id 
        // 2. 声明的div 有问题，找不到装商品的大盒子

        // 清空选中的商品 --> 需要清除本地数据  可以用事件委托做
        if (target.className == 'del2') {
            var checks = document.querySelectorAll('.check01') //获取每个商品的选择框
            let div = target.parentElement.parentElement.parentElement.previousElementSibling.children.item.id //商品最大的盒子
            let data3 = JSON.parse(localStorage.getItem('goods')) //每次操作数据都需要重新请求
                // console.log(div);  undefined
                // console.log(div.index);  报错
            ...
            for (var i = 0; i < checks.length; i++) {
                ...
                if (checks[i].checked) {
                    ...
                    data3.forEach((item, index) => {
                        console.log(item.id);
                        item.id === div.index && data3.splice(index, 1)
                    })
    ...
    // 去结算
    var settleAccounts = document.querySelector('.myCar_right>span')
    settleAccounts.onclick = function() {
        var checks = document.querySelectorAll('.check01') //获取每个商品的选择框
            // console.log(checks);
        for (var i = 0; i < checks.length; i++) {
